...
public class PDFGenerater : MonoBehaviour {
	...
	void OnGUI () {
        ...
        {
            CreateFromFolder (Application.streamingAssetsPath);
        }
        else if(GUILayout.Button("CreateFromRawdataFile"))
        {
            CreateFromRawdataFile("C:/RawData.txt", "C:/outpdf.pdf");
        }
    }
	...
	void CreateFromRawdataFile(string sourceTxtPath, string destinationPDFPath){
		...
		pdfwriter = iTextSharp.text.pdf.PdfWriter.GetInstance (pdfdoc,new FileStream(destinationPDFPath,FileMode.OpenOrCreate));
	...
	void CreateFromFolder(string folderPath){
		...
		string[] paths = Directory.GetFiles (folderPath + "/Images");
